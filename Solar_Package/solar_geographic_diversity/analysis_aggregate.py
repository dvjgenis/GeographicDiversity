# GeographicDiversity_Package/Solar_Package/solar_geographic_diversity/analysis_aggregate.py

import pandas as pd

def aggregate_pair_analysis(df1, df2):
    """
    Computes aggregate production metrics for two sites.

    Parameters:
        df1 (pd.DataFrame): DataFrame for site 1. Must contain 'Datetime' and 'Power_Output'.
        df2 (pd.DataFrame): DataFrame for site 2. Must contain 'Datetime' and 'Power_Output'.

    Returns:
        dict: Metrics including counts of nonzero production hours, means, standard deviations,
              coefficients of variation (CV), ratios and percentage differences compared to individual site averages,
              and the total number of hours.
    """
    # Align the two DataFrames on the Datetime index
    df1_aligned, df2_aligned = df1.set_index('Datetime').align(df2.set_index('Datetime'), join='inner')
    
    # Calculate aggregate production (sum of the two sites)
    agg_production = df1_aligned['Power_Output'] + df2_aligned['Power_Output']

    # Count hours with nonzero output
    count_site1 = (df1_aligned['Power_Output'] > 0).sum()
    count_site2 = (df2_aligned['Power_Output'] > 0).sum()
    ...
    # Calculate means
    mean_site1 = df1_aligned['Power_Output'].mean()
    mean_site2 = df2_aligned['Power_Output'].mean()
    mean_agg = agg_production.mean()
    
    # Calculate standard deviations
    std_site1 = df1_aligned['Power_Output'].std()
    std_site2 = df2_aligned['Power_Output'].std()
    ...
    # Calculate count difference of count agg and count of individual means
